...
def test_get_relevant_terms(db: Session):
    ...
    t1 = GlossaryTerm(project_id=project.id, source_term="Apple", translated_term="Яблоко", category="other", status=TermStatus.APPROVED)
    t2 = GlossaryTerm(project_id=project.id, source_term="Banana", translated_term="Банан", category="other", status=TermStatus.APPROVED)
    t3 = GlossaryTerm(project_id=project.id, source_term="Cherry", translated_term="Вишня", category="other", status=TermStatus.APPROVED)
    t4 = GlossaryTerm(project_id=project.id, source_term="Draft", translated_term="Черновик", category="other", status=TermStatus.PENDING)
...
def test_get_relevant_terms_case_insensitive(db: Session):
    ...
    t1 = GlossaryTerm(project_id=project.id, source_term="magic sword", translated_term="меч", category="artifact", status=TermStatus.APPROVED)
    db.add(t1)
    db.commit()
