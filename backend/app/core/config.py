...
class Settings(BaseSettings):
    ...
    # ──────────────────────────────────────────────
    GEMINI_API_KEYS_RAW: str = Field(..., description="Comma-separated Gemini API keys (each key = separate GCP project)")
    GEMINI_API_RESET_TIMEZONE: str = Field(default="America/Los_Angeles", description="Timezone for daily RPD reset (Mountain View, CA)")
    GEMINI_API_COOLDOWN_MINUTES: int = Field(default=5, description="Soft cooldown minutes for a key after 429 from Google")

    # ──────────────────────────────────────────────
    # Per-task Model Selection (defaults, overridable per-project in DB)
    # ──────────────────────────────────────────────
    GEMINI_MODEL_EXTRACTION: str = Field(default="gemini-3.1-flash-lite-preview", description="Model for term extraction")
    GEMINI_MODEL_TRANSLATION: str = Field(default="gemini-3-flash-preview", description="Model for translation")
    GEMINI_MODEL_SUMMARIZATION: str = Field(default="gemini-3.1-flash-lite-preview", description="Model for context summarization")
    GEMINI_MODEL_RELATIONSHIPS: str = Field(default="gemini-3.1-flash-lite-preview", description="Model for relationship analysis")
    GEMINI_MODEL_EMBEDDING: str = Field(default="gemini-embedding-2-preview", description="Model for embeddings")
    ...
    # ──────────────────────────────────────────────
    GEMINI_THINKING_EXTRACTION: str = Field(default="medium", description="Thinking level for extraction tasks")
    GEMINI_THINKING_TRANSLATION: str = Field(default="high", description="Thinking level for translation tasks")
    GEMINI_THINKING_SUMMARIZATION: str = Field(default="medium", description="Thinking level for summarization")
    GEMINI_THINKING_RELATIONSHIPS: str = Field(default="medium", description="Thinking level for relationship analysis")
    ...
    # ──────────────────────────────────────────────
    GEMINI_MAX_TOKENS_EXTRACTION: int = Field(default=8192, description="Max output tokens for extraction")
    GEMINI_MAX_TOKENS_TRANSLATION: int = Field(default=65536, description="Max output tokens for translation (Gemini 3 Flash limit)")
    GEMINI_MAX_TOKENS_SUMMARIZATION: int = Field(default=2048, description="Max output tokens for summarization")
