...
def upload_chapters_from_file(
    ...
    try:
        ...
        # Using safe_finditer to prevent ReDoS from user-supplied patterns
        try:
            matches = safe_finditer(f"\\n({chapter_pattern})", content, flags=re.IGNORECASE)
        except TimeoutError:
            raise HTTPException(
                status_code=400,
                detail="Regex processing timed out. The pattern is too complex or the file is too large."
            )
