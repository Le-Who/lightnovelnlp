...
"""
    ...
    def translate_with_review(
        ...
        logger.info(f"[FEEDBACK_LOOP] Starting translate+review for chapter {chapter_id}")
        ...
        if "error" in review_result:
            ...
            logger.warning(f"[FEEDBACK_LOOP] Review failed for chapter {chapter_id}: {review_result}")
        ...
        if passed or not violations or max_retries < 1:
            logger.info(f"[FEEDBACK_LOOP] Chapter {chapter_id} passed review (score={review.get('score')}). No retranslation needed.")
            return {**translate_result, "review": review, "audit": audit, "retranslated": False}

        # Step 3 — retranslate with corrections
        logger.info(f"[FEEDBACK_LOOP] Chapter {chapter_id} has {len(violations)} violation(s). Triggering correction pass.")
        ...
        try:
            ...
            logger.warning(f"[FEEDBACK_LOOP] Verification review failed (non-critical): {e}")

        audit["retranslated"] = True
        audit["final_score"] = final_score

        logger.info(f"[FEEDBACK_LOOP] Correction complete for chapter {chapter_id}. Score: {audit['initial_score']} → {final_score}")
    ...
    def _retranslate_with_corrections(
        ...
        def _fmt_violation(v: Dict[str, Any]) -> str:
            excerpt_suffix = f" | excerpt: {v['excerpt'][:80]}" if v.get('excerpt') else ''
            return (
...
"""
    ...
    def _parse_review_json(raw: str) -> Dict[str, Any]:
        ...
        """
        if not raw:
            return {"score": 0, "passed": True, "violations": [], "style_notes": "", "parse_error": "empty response"}
        ...
        try:
            ...
            logger.warning(f"[REVIEW] Failed to parse review JSON: {e}. Raw: {raw[:200]}")
            return {
                "score": 5,
                "passed": True,
                "violations": [],
    ...
    def _get_relevant_relationships(db: Session, glossary_terms: List[GlossaryTerm]) -> List[Dict[str, Any]]:
        ...
        from app.models.glossary import TermRelationship  # local import avoids circular dep
